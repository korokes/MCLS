...
def get_args():
    ...
    # Add data arguments
    parser.add_argument('--data', default='data-bin/how2mcls', help='path to data directory') #data-bin/how2small
    ...
    def load_enc_dec_checkpoint(args, model):
        ...
        if os.path.isfile(decoder_checkpoint_path):
            ...
            if int(args.kd_type) == 0: #encoder-level kd
                encoder_state_dict = {k:v for k,v in encoder_state_dict.items() if ('embed' not in k) and ('layers.0' not in k) and ('layers.1' not in k)}
