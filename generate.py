...
from seq2seq.generator import MultimodalSequenceGenerator
...
def get_args():
    ...
    parser.add_argument('--video_file', default='data/demo_data/test_action.txt', help='name of video file')
    parser.add_argument('--video_dir', default='data/demo_data/video_action_features', help='path of video features')
    parser.add_argument('--checkpoint-path', default='checkpoints/model_s3/checkpoint_last.pt', help='path to the model file')
...
def main(args):
    ...
    test_dataset = How2Dataset(
              src_file=os.path.join(args.data, '{}_pt.{}'.format(args.test_name, 'tran')),
              tgt_file=os.path.join(args.data, '{}.{}'.format(args.test_name, 'desc')),
    ...
    translator = MultimodalSequenceGenerator(
        model, all_en_dict, beam_size=args.beam_size, maxlen=args.max_len, stop_early=eval(args.stop_early),
    ...
    for i, sample in enumerate(progress_bar):
        ...
        for i, (sample_id, hypos) in enumerate(zip(sample['id'].data, hypos)):
            src_tokens = utils.strip_pad(sample['src_tokens'].data[i, :], all_en_dict.pad_idx)
            has_target = sample['tgt_tokens'] is not None
            target_tokens = utils.strip_pad(sample['tgt_tokens'].data[i, :], all_en_dict.pad_idx).int().cpu() if has_target else None

            src_str = all_en_dict.string(src_tokens, args.remove_bpe)
            target_str = all_en_dict.string(target_tokens, args.remove_bpe) if has_target else ''
            ...
            for i, hypo in enumerate(hypos[:min(len(hypos), args.num_hypo)]):
                ...
                if has_target and i == 0:
                    ...
                    target_tokens = all_en_dict.binarize(target_str, word_tokenize, add_if_not_exist=True)
                    print(target_tokens)
