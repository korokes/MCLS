...
def get_args():
    ...
    parser.add_argument('--seed', default=91, type=int, help='pseudo random number generator seed')
    ...
    # Add data arguments
    parser.add_argument('--data', default='data-bin/how2mcls', help='path to data directory') 
    ...
    parser.add_argument('--num-workers', default=16, type=int, help='number of data workers')

    # Add model arguments
    parser.add_argument('--arch', default='CAX2transformer', choices=ARCH_MODEL_REGISTRY.keys(), help='model architecture')
    ...
    parser.add_argument('--log-file', default='./log_dir/log_s3.txt', help='path to save logs') 
    parser.add_argument('--save-dir', default='checkpoints/model_s3', help='path to save checkpoints') 
    parser.add_argument('--daencoder-dir', default='checkpoints/model_s2', help='path to save checkpoints')
    parser.add_argument('--decoder-dir', default='checkpoints/model_s1', help='path to save checkpoints')
    parser.add_argument('--kd_type', default='0', help='type of knowledge distillation in training stage 2, choose 0 (encoder-level) or 1 (vocab-level)')
...
def main(args):
    ...
    def load_data(split,video_file=None, video_dir=args.video_dir):
          return How2Dataset(
              src_file=os.path.join(args.data, '{}_pt.{}'.format(split, 'tran')),  
              tgt_file=os.path.join(args.data, '{}.{}'.format(split, 'desc')),
              all_dict=all_en_dict,
    ...
    def load_enc_dec_checkpoint(args, model):
        daencoder_checkpoint_path = os.path.join(args.daencoder_dir, args.restore_file)
        decoder_checkpoint_path = os.path.join(args.decoder_dir, args.restore_file)

        model_dict =  model.state_dict()
        if os.path.isfile(daencoder_checkpoint_path):
            daencoder_model = torch.load(daencoder_checkpoint_path, map_location=lambda s, l: default_restore_location(s, 'cpu'))
            _daencoder_model =  {k:v for k,v in daencoder_model['model'].items() if 'source' in k}
            daencoder_state_dict = {k[7:]:v for k,v in _daencoder_model.items() if k[7:] in model_dict.keys()}
        if os.path.isfile(decoder_checkpoint_path):
      
            decoder_model = torch.load(decoder_checkpoint_path, map_location=lambda s, l: default_restore_location(s, 'cpu'))
            decoder_state_dict = {k:v for k,v in decoder_model['model'].items() if 'decoder' in k}
            
            encoder_state_dict = {k:v for k,v in decoder_model['model'].items() if 'encoder.' in k}
         
            if int(args.kd_type) == 0: #encoder-level kd
                #encoder_state_dict = {k:v for k,v in encoder_state_dict.items() if ('embed' not in k) and ('layers.0' not in k) and ('layers.1' not in k)} #e2
                encoder_state_dict = {k:v for k,v in encoder_state_dict.items() if ('embed' not in k) and ('layers.0' not in k) and ('layers.1' not in k) and ('layers.2' not in k) and ('layers.3' not in k)} #e4
                #encoder_state_dict = {k:v for k,v in encoder_state_dict.items() if ('embed' not in k) and ('layers.0' not in k) and ('layers.1' not in k) and ('layers.2' not in k)} #e3
                #encoder_state_dict = {k:v for k,v in encoder_state_dict.items() if ('embed' not in k) and ('layers.0' not in k)} #e1
            elif int(args.kd_type) == 1: #vocab-level kd
                encoder_state_dict = {k:v for k,v in encoder_state_dict.items() if ('embed' not in k)}
            else:
                print('KD type error')
            
        #print('_daencoder parm',len(_daencoder_model.keys()))     
        print('daencoder_state_dict',len(daencoder_state_dict.keys())) 
        print('decoder_state_dict',len(decoder_state_dict.keys()))
        print('encoder_state_dict',len(encoder_state_dict.keys()))
        print('model_dict',len(model_dict.keys()))
 
        model_dict.update(decoder_state_dict)
        model_dict.update(encoder_state_dict)  
        model_dict.update(daencoder_state_dict)  
        
        model.load_state_dict(model_dict)
        logging.info('Loaded daencoder checkpoint {}'.format(daencoder_checkpoint_path))
        logging.info('Loaded decoder checkpoint {}'.format(decoder_checkpoint_path))
        return decoder_state_dict
    
    load_enc_dec_checkpoint(args, model)
    #load_dec_checkpoint(args, model)
    last_epoch =  -1
