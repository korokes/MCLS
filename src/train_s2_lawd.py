...
def get_args():
    ...
    parser.add_argument('--max-tokens', default=1024, type=int, help='maximum number of tokens in a batch')
    ...
    # Add model arguments
    parser.add_argument('--arch', default='DTWtransformer', choices=ARCH_MODEL_REGISTRY.keys(), help='model architecture')
...
def main(args):
    ...
    def load_all_checkpoint(args, model):
        checkpoint_path = os.path.join(args.target_dir, args.restore_file)
        if os.path.isfile(checkpoint_path):
            target_model = torch.load(checkpoint_path, map_location=lambda s, l: default_restore_location(s, 'cpu'))
            model_dict = model.state_dict()
            state_dict = {k:v for k,v in target_model['model'].items() if k in model_dict.keys()}
            model_dict.update(state_dict)
            model.load_state_dict(model_dict)
            logging.info('Loaded checkpoint {}'.format(checkpoint_path))
            return state_dict
    ...
    #load_all_checkpoint(args, model)
    last_epoch =  -1
    ...
    for epoch in range(last_epoch + 1, args.max_epoch):
        ...
        for i, sample in enumerate(progress_bar):
            ...
            loss = model(sample['src_en_tokens'], sample['src_en_lengths'], sample['src_pt_tokens'], sample['src_pt_lengths']) 
            optimizer.zero_grad()
            loss = loss.mean()
            loss.backward()
        ...
        valid_loss = validate(args, model, valid_dataset, epoch)
