...
@register_model('APDtransformer')
class APDTransformerModel(Source2TargetModel):
    ...
    def build_model(cls, args, all_en_dict, src_pt_dict):
        ...
        source_encoder = APDTransformerEncoder(args, src_pt_dict, src_embed_tokens)
        target_encoder = TransformerEncoder(args, all_en_dict, shared_embed_tokens)
        return APDTransformerModel(source_encoder, target_encoder)


class APDTransformerEncoder(Seq2SeqEncoder):
...
@register_model_architecture('APDtransformer', 'APDtransformer')
