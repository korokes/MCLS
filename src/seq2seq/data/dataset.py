...
class How2Dataset(Dataset):
    def __init__(self, src_file, video_file, video_dir, tgt_file, all_dict):
        ...
        with open(src_file, 'rb') as f:
            self.src_dataset = pickle.load(f)
            self.src_dataset = self.src_dataset
            self.src_sizes = np.array([len(tokens) for tokens in self.src_dataset])
        with open(tgt_file, 'rb') as f:
            self.tgt_dataset = pickle.load(f)
            self.tgt_dataset = self.tgt_dataset
            self.tgt_sizes = np.array([len(tokens) for tokens in self.tgt_dataset])
        with open(video_file, 'r',encoding='utf8') as f:
            video_samples = [line.strip().split('/')[-1]+'.npy' for line in f]
...
class How2MCLS_Dataset(Dataset):
    def __init__(self, src_en_file, src_pt_file, video_file, video_dir, tgt_en_file, all_en_dict, src_pt_dict):
        ...
        with open(src_en_file, 'rb') as f:
            ...
            self.src_en_dataset = self.src_en_dataset
        ...
        with open(src_pt_file, 'rb') as f:
            ...
            self.src_pt_dataset = self.src_pt_dataset
        ...
        with open(tgt_en_file, 'rb') as f:
            ...
            self.tgt_en_dataset = self.tgt_en_dataset
