...
def get_args():
    ...
    parser.add_argument('--train_video_file', default='data/demo_data/pretrain_action.txt', help='name of train video file') #train pretrain
    ...
    # Add model arguments
    parser.add_argument('--arch', default='CA2transformer', choices=ARCH_MODEL_REGISTRY.keys(), help='model architecture')
    ...
    parser.add_argument('--log-file', default='./log_dir/log_s1.txt', help='path to save logs') 
    parser.add_argument('--save-dir', default='checkpoints/model_s1', help='path to save checkpoints') 
...
def main(args):
    ...
    # Load dictionaries
    all_dict = Dictionary.load(os.path.join(args.data, 'dict.all_en')) 
    ...
    def load_data(split,video_file=None, video_dir=args.video_dir):
          return How2Dataset(
              src_file=os.path.join(args.data, '{}_en.{}'.format(split, 'tran')), 
              tgt_file=os.path.join(args.data, '{}.{}'.format(split, 'desc')),
              all_dict=all_dict,
              video_file=video_file, 
              video_dir=video_dir)  

    train_dataset = load_data(split='pretrain',video_file=args.train_video_file)  #pretrain train 
    valid_dataset = load_data(split='val',video_file=args.val_video_file)

    # Build model and criterion
    model = models.build_model(args, all_dict, all_dict).cuda()
