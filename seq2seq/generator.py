...
class SequenceGenerator(object):
    ...
    def _generate(self, src_tokens, src_lengths):
        ...
        for step in range(maxlen + 1):  # Extra step for EOS marker
            ...
            lprobs, avg_attn_scores = self._decode(tokens[:, :step + 1], encoder_outs, incremental_states, log_probs=True,beam_size=beam_size)
    ...
    def _decode(self, tokens, encoder_out, incremental_states, log_probs, beam_size):
...
class MultimodalSequenceGenerator(object):
    def __init__(
        self, model, tgt_dict, beam_size=1, minlen=1, maxlen=None, stop_early=True, normalize_scores=True,
        len_penalty=1, unk_penalty=0,
    ):
    ...
    def _generate(self, src_tokens, src_lengths, video_inputs):
        ...
        for step in range(maxlen + 1):  # Extra step for EOS marker
            ...
            if step < maxlen:
               ...
               # cand_indices = torch.tensor(cand_indices, dtype=torch.float64)
               # print(cand_indices, type(cand_indices),cand_indices.size())
               # print(self.vocab_size)
               # print(cand_beams, cand_beams.size())
               # exit()
