...
'''
Training stage2, encoder-level KD
'''

@register_model('DAtransformer')
class DATransformerModel(Source2TargetModel):
    ...
    def build_model(cls, args, all_en_dict, src_pt_dict):
        ...
        source_encoder = DATransformerEncoder(args, src_pt_dict, src_embed_tokens)
        target_encoder = TransformerEncoder(args, all_en_dict, shared_embed_tokens)
        return DATransformerModel(source_encoder, target_encoder)


class DATransformerEncoder(Seq2SeqEncoder):
...
class TransformerEncoder(Seq2SeqEncoder):
    ...
    def forward(self, encoder_out, src_tokens, src_lengths):
        ...
        #print('before', encoder_out['encoder_out'].size(), x.size())
        adapt_pool = nn.AdaptiveAvgPool1d(encoder_out['encoder_out'].size()[0]) # encoder_out [494, 8, 512] Avg/Max
        x = adapt_pool(x.transpose(0,2)).transpose(0,2)
        #print('after', encoder_out['encoder_out'].size(), x.size())
...
@register_model_architecture('DAtransformer', 'DAtransformer')
