...
@register_model('CA2transformer')
class CATransformerModel(MMSeq2SeqModel):
    ...
    def build_model(cls, args, all_en_dict, src_pt_dict):
        ...
        encoder = TransformerEncoder(args, all_en_dict, shared_embed_tokens)
        decoder = CATransformerDecoder(args, all_en_dict, shared_embed_tokens)
        return CATransformerModel(encoder, decoder)
...
class CATransformerDecoder(Seq2SeqDecoder):
    def __init__(self, args, dictionary, embed_tokens):
        super().__init__(dictionary)
        embed_dim = embed_tokens.embedding_dim
        self.padding_idx = embed_tokens.padding_idx
        ...
        self.layers[0] = CATransformerDecoderLayer(args)
        self.layer_norm = nn.LayerNorm(embed_dim)
...
class CATransformerDecoderLayer(nn.Module):
    def __init__(self, args):
        super().__init__()
        self.embed_dim = args.decoder_embed_dim
        self.dropout = args.dropout
...
@register_model_architecture('CA2transformer', 'CA2transformer')
